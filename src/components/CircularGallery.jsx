...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    this.mediasImages = (items && items.length ? items : []).concat(items && items.length ? items : []);
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = DEFAULT_FONT, fontUrl, scrollSpeed = 2, scrollEase = 0.05, onItemClick }) {
  ...
  useEffect(() => {
    if (!canUseWebGL()) return; // fallback will render instead
  ...
  if (typeof window !== 'undefined' && !canUseWebGL()) {
    return (
      <div
        ref={fallbackRef}
        className="circular-gallery-fallback"
