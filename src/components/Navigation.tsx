...
const Navigation = () => {
  ...
  return (
    <>
      <nav ref={navRef} className="fixed top-0 left-0 right-0 z-40 glass border-b border-border/40">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div ref={menuRef} className="hidden md:flex items-center space-x-8">
              {navItems.map((item) => (
                <button key={item.id} onClick={() => scrollToSection(item.id)} className="text-foreground/70 hover:text-primary transition-colors duration-300 font-light">
                  {item.name}
                </button>
              ))}
      ...
      <div ref={mobileMenuRef} className="fixed top-0 right-0 w-full h-full bg-background/95 backdrop-blur-lg z-50 transform translate-x-full md:hidden">
        <div className="flex items-center justify-between p-6 border-b border-border/40">
          <h2 className="text-2xl font-light text-primary">Portfolio</h2>
        ...
        <div className="flex flex-col space-y-6 p-6 mt-8">
          {navItems.map((item) => (
            <button key={item.id} onClick={() => scrollToSection(item.id)} className="menu-item text-left text-xl text-foreground/70 hover:text-primary transition-colors duration-300">
              {item.name}
            </button>
          ))}
